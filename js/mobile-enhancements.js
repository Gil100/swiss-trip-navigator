...
// פונקציית בחירת יום מתוקנת לקובץ mobile-enhancements.js
// החלף את הפונקציה addFixedDaySelector הקיימת בזו:
...
function fillDaysMenu() {
    ...
    // גישה ישירה ל-appState
    if (!window.appState || !window.appState.itineraryData || !window.appState.itineraryData.days) {
    console.warn('appState לא זמין או חסרים נתוני מסלול');
    
      // נסה לגשת לנתוני המסלול בדרך אחרת
    ...
    const currentIndex = window.appState.currentDayIndex;
    ...
    li.addEventListener('click', function() {
        ...
        // הפעלת פונקציית הצגת היום
        if (typeof window.showCurrentDay === 'function') {
        window.showCurrentDay();
        } else {
          // גישה ישירה לאלמנטים אם הפונקציה לא זמינה
        ...
        if (dayCounter) {
            dayCounter.textContent = `${index + 1} / ${days.length}`;
        }
